...
function MyApp({ Component, pageProps }) {
  ...
  const refreshToken = async () => {
    const res = await axios.post('https://api.maaambeyeducation.in/api/user/refresh-token', {}, {
      headers: {
  ...
  useEffect(() => {
    console.log("date", Date.now() / 1000)
    if (getToken()) {
      if (jwtDecode(getToken()?.toString()).exp < Date.now() / 1000 && router.pathname.includes('admin')) {
        refreshToken()
      }
    }
  }, [getToken()])
